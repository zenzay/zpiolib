#!/usr/bin/env python3
# -*- coding: utf-8 -*-

"""

    Zenzay's Pi Input/Output Library.

    zpiolib.led.pwm_wave_led module

    Created by Jens Hansen 2019.

"""

from ..gpio.pwm_wave import PWM_Wave

class PWM_Wave_LED(PWM_Wave):
    def __init__(self, pi, pin,):
        super().__init__(pi, pin)

    def turn_on(self, brightness=1, transition=1, delay=0, wait=True):
        if isinstance(brightness, (list,tuple)):
            self.set_pulse(brightness, transition, delay)
            self.start_wave(wait=wait, repeat=True)
        else:
            self.set_value(value=brightness, transition=transition, wait=wait)

    def turn_off(self, transition=1, wait=True):
        self.stop_wave()
        self.set_value(value=0, transition=transition, wait=wait)
